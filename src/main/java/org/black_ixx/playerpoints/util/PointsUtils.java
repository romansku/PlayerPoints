...
public final class PointsUtils {
    ...
    public static void getPlayerByName(String name, Consumer<Tuple<UUID, String>> callback) {
        ...
        if (SettingKey.USE_EXACT_SEARCH.get()) {
            applyCallbackWithExactSearch(name, callback);
            return;
        }
    ...
    private static void applyCallbackWithExactSearch(String name, Consumer<Tuple<UUID, String>> callback) {
        PlayerPoints plugin = PlayerPoints.getInstance();
    ...
    private static <T> CompletableFuture<T> callAsync(Callable<T> callable) {
        ...
        PlayerPoints.getInstance().getScheduler().runTaskAsync(() -> {
            try {
                ...
                future.complete(result); // Передаем результат в future
            } catch (Exception e) {
                future.completeExceptionally(e); // Передаем ошибку
            }
        });
